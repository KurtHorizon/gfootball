...
def discount_reward(r, gamma,final_r):
    ...
    return discounted_r
'''
def discount_reward(r, type,gamma,final_r):
    discounted_r = np.zeros_like(type)
    running_add = final_r
    for t in reversed(range(0, len(r))):
        running_add = running_add * gamma + r[t]
...
def main():
    ...
    if herit == False:
        ...
        value_network  = torch.load('./param/net_param/critic_net2pkl')
        actor_network  = torch.load('./param/net_param/actor_net2.pkl')
    #init optim
    ...
    for step in range(STEP):
        ...
        qs = Variable(torch.Tensor(discount_reward(rewards,vs,_gamma,final_r)))
        advantages = qs - vs
        #policy gradient: loss= -log(prob)*vt
        #actor_network_loss = - torch.mean(torch.sum(log_softmax_actions*actions_var,1)* advantages)

        old_prob = actor_network(states_var)
        ratio = torch.exp(torch.log(actions_var + 1e-10)-old_prob)
        p1 = ratio * advantages
        p2 = torch.clamp(ratio, 1 - clip_param, 1 + clip_param) * advantages

        actor_network_loss = -torch.mean(torch.min(p1, p2))
        ...
        if (step+1)%50==0:
            ...
            if save_net == True and ruleBase==False:
                    torch.save(actor_network, './param/net_param/actor_net2.pkl')
                    torch.save(value_network, './param/net_param/critic_net2.pkl')
